...
from paella.base.util import ujoin, makepaths, filecopy
...
class TraitTemplate(_TraitRelation):
    ...
    def insert_template(self, data, templatefile):
        ...
        f = self.cmd.file(self.conn)
        insert_data['templatefile'] = f.name
        f.write(templatefile.read())
        f.close()
    ...
    def update_template(self, data, templatefile):
        ...
        row = self.cmd.select_row(fields=['templatefile'], clause=clause)
        self.conn.removefile(row.templatefile.name)
        f = self.cmd.file(self.conn)
        update['templatefile'] = f.name
        f.write(templatefile.read())
        f.close()
    ...
    def templatefile(self, package, template):
        clause = self._clause(package, template)
        return self.cmd.open_file(self.conn, 'templatefile', clause)

    def remove_templatefile(self, package, template):
        clause = self._clause(package, template)
        self.cmd.delete_file(self.conn, 'templatefile', clause)
            
    def save_template(self, package, template, templatefile):
        clause = self._clause(package, template)
        self.cmd.update_file(self.conn, 'templatefile', clause, templatefile)
...
class TraitScript(_TraitRelation):
    ...
    def scriptfile(self, name):
        ...
        return self.cmd.open_file(self.conn, 'scriptfile', clause)

    def save_script(self, name, scriptfile):
        clause = self._clause(name)
        self.cmd.update_file(self.conn, 'scriptfile', clause, scriptfile)

    def remove_scriptfile(self, name):
        clause = self._clause(name)
        self.cmd.delete_file(self.conn, 'scriptfile', clause)
    ...
    def read_script(self, name):
        return self.scriptfile(name).read()
    
    def insert_script(self, name, scriptfile):
        insert_data = {'trait' : self.current_trait,
                       'script' : name}
        f = self.cmd.file(self.conn)
        insert_data['scriptfile'] = f.name
        f.write(scriptfile.read())
        f.close()
        self.cmd.insert(data=insert_data)

    def update_script(self, name, scriptfile):
        clause = self._clause(name)
        row = self.cmd.select_row(fields=['scriptfile'], clause=clause)
        self.conn.removefile(row['scriptfile'].name)
        f = self.cmd.file(self.conn)
        update = dict(scriptfile=f.name)
        f.write(scriptfile.read())
        f.close()
        self.cmd.update(data=update, clause=clause)
        self.reset_clause()
...
class Trait(object):
    ...
    def insert_trait(self, path, suite=None):
        ...
        for template, data in trait.templates.items():
            print template, data
